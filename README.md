...
| 任务5 样本增强与金标导入 | `scripts/import_excel_fewshot_bank.py` + `engine/classification/fewshot.py`；提示词规范消融 `scripts/eval_task5_prompt_ab.py` | 评测集外 few-shot；裸标签列表 vs 判定标准提示词 A/B（本地重跑生成报告，不入库） |
